...
module bus_outputs (
    ...
    input wire BUS_FILE_READY_L,          // real drive is ready, heads loaded
    input wire BUS_WRITE_SEL_ERR_L,       // error when attempting write
    input wire BUS_90S_RELAY_L,           // 90 second relay while drive purges
...
begin : BUSOUTPUTS // block name
    ...
    BUS_ACC_GO_DRIVE_DRIVE_L       <=         1'b1;
    BUS_10_20_DRIVE_L              <=         1'b1;
    ...
    BUS_ACC_REV_DRIVE_L            <=        1'b1;

    // signals sent to 1130 regardless of mode
    BUS_WT_CLOCKB_CTRL_L           <=        (BUS_WT_CLOCKB_EMUL_L | ~Cart_Ready);
    BUS_RD_DATA_CTRL_L             <=        (~BUS_RD_DATA_H);
    BUS_RD_CLK_CTRL_L              <=        (~BUS_RD_CLK_H);

    // never sent to drive
    BUS_HEAD_SELECT_DRIVE_L        <=        1'b1;
    BUS_RD_GATE_DRIVE_L            <=        1'b1;
    BUS_WT_GATE_DRIVE_L            <=        1'b1;
    BUS_WT_DATA_CLK_DRIVE_L        <=        1'b1;
    ...
    BUS_90S_RELAY_CTRL_L           <=        real_drive == 1'b1   // don't pass along if we are not ready to go
                                               ? (BUS_90S_RELAY_L & Selected_Ready)
                                               : (BUS_90SEC_RELAY_EMUL_L | ~Cart_Ready);

    BUS_FILE_READY_CTRL_L          <=        real_drive == 1'b1
                                               ? (BUS_FILE_READY_L | ~Cart_Ready)        
                                               : (~Cart_Ready | ~Selected);
