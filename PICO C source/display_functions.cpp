...
static FIL fil;
static uint8_t sectordata[MAX_SECTOR_SIZE];  // used hear to read in BMP for display
...
uint8_t* big_disk = digit_dm;
uint8_t* no_disk = digit_em;
...
void display_splash_screen()
{
    ...
    ssd1306_invert(&disp, 0);
    edisplay.display_inverted = false;
    ssd1306_draw_string(&disp, 12,  5, 2, (char*) "Virtual");
    ssd1306_draw_string(&disp, 30, 20, 2, (char*) "2315");
    ssd1306_draw_string(&disp, 10, 35, 2, (char*) "Cartridge");
    ssd1306_draw_string(&disp, 15, 50, 2, (char*) "Facility");
    ssd1306_show(&disp);
    sleep_ms(1000);
...
void display_drive_address(int drv_addr, bool fixed, char *image_name, bool showicon)
{
    uint8_t *digit_pointer;

    // if we have a disk file opened, then display the 2315 image otherwise blank
    if (showicon){
        // image of 2315 cartridge
        digit_pointer = big_disk; 
    } else {
        // blank image
        digit_pointer = no_disk;
    }
    ...
    ssd1306_bmp_show_image_with_offset(&disp, sectordata, 446, 0, 0);

    // image name
    x_coord = DISPLAY_WIDTH / 2 - (strlen(image_name) * ssd1306_get_font_width(2)) / 2;
    ssd1306_draw_string(&disp, x_coord, 52, 2, image_name);
    ssd1306_show(&disp);
}

void manage_display_timers(Disk_State* ddstate, bool showicon)
{
    if(edisplay.display_message_timer > 0){
        edisplay.display_invert_timer = DISPLAY_INVERT_TIME;
        edisplay.display_message_timer--;
        if(edisplay.display_message_timer == 0){
            display_drive_address(ddstate->Drive_Address, ddstate->mode_RK05f, ddstate->File_Ready ? ddstate->imageName : (char *)"", showicon);
            edisplay.display_invert_timer = DISPLAY_INVERT_TIME;
        }
    }
    else{
