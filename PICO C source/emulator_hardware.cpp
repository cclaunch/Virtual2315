...
#define CMD_INTERRUPT 4
#define ALT_INTERRUPT 5
...
void initialize_gpio()
{
    ...
    // command interrupt pin 4, driven by FPGA, used to log read, write or seek events
    gpio_init(CMD_INTERRUPT);
    gpio_set_dir(CMD_INTERRUPT, GPIO_IN);

    // alternate command interrupt pin 5, driven by FPGA, used to log read, write or seek events
    gpio_init(ALT_INTERRUPT);
    gpio_set_dir(ALT_INTERRUPT, GPIO_IN);
    ...
    if (get_power_fail()) {
        printf("Power fail indication is on!\r\n");
    }
