...
int intermediate_result;
    ...
    switch(dstate->run_load_state){
        ...
        case RLST0:
            ...
            if (get_power_fail() != dstate->saw_fail) {
                if (dstate->saw_fail == false) {
                    display_error((char *) "pwr fail", (char *) "detected");
                    dstate->saw_fail = true;
                    dstate->run_load_state = RLST18;
                    sleep_ms(1000);
                } else {
                    dstate->saw_fail = false;
                }
                break;
            }
            ...
            if(dstate->rl_switch == 1){
                ...
                } else {  // warn that drive must be unlocked first
                    printf("LOAD requested but unlock lamp is not lit\rn");
                    display_status((char *) "Must be", (char *) "unlocked");
                }
        ...
        case RLST1:
            ...
            if(is_card_present()){
                printf("Mini 2315 present and detected\r\n");
                display_status((char *) "2315", (char *) "inserted");
                dstate->run_load_state = RLST2; // if the microSD card is inserted then advance to RLST2
            }
            else {
                //error_code = 1;
                printf("*** ERROR, mini 2315 is not inserted or motor switch off\r\n");
                display_error((char *) "no 2315", (char *) "inserted");
                dstate->run_load_state = RLST18;
            }
            break;
        ...
        // wait for disk to declare the drive ready for input output
             ...
             // we wait here until Load/Unload switch is turned off
                ...
                if (dstate->rl_switch == 0) {
                   printf("Requested unload when not ready\r\n");
                   clear_cart_ready();
        ...
        case RLST10:
            // Loaded and running state. 
            //
            // this is the resting location when we are operating as a disk drive
            //
            microSD_LED_on();
            ...
            if (get_power_fail()) {
                display_error((char *) "pwr fail", (char *) "unload");
